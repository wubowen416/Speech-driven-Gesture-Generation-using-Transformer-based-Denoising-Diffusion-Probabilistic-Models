...
# import embedding_net
from .tcn import TemporalConvNet
from .ResNetSE34V2 import *
from .ResNetBlocks import *

class Hierarchical_WavEncoder(nn.Module):
    def __init__(self, args, z_obj, pose_level, nOut=32):
        super().__init__()
        num_filters = [32, 64, 128, 256]
        self.feat_extractor = ResNetSE(args, SEBasicBlock, [3, 4, 6, 3], num_filters, nOut=nOut, pose_level=pose_level, z_obj=z_obj, n_mels=128)

    def forward(self, audio_spectrum, vid_indices):
        audio_spectrum = audio_spectrum.unsqueeze(1)
        out = self.feat_extractor(audio_spectrum, vid_indices)
...
class TextEncoderTCN(nn.Module):
    ...
    def __init__(self, args, n_words, embed_size=300, pre_trained_embedding=None,
        ...
        if pre_trained_embedding is not None:  # use pre-trained embedding
...
class Hierarchical_PoseGenerator(nn.Module):
    def __init__(self, args, pose_dim, n_words, word_embed_size, word_embeddings, z_obj=None):
        ...
        elif self.input_context == 'audio' or self.input_context == 'text':
            self.in_size = 32 + pose_dim + 1  # audio or text only
        else:
            self.in_size = 32 + 32 + pose_dim + 1  # audio or text only
        # self.audio_encoder = WavEncoder(args, z_obj=z_obj, nOut=32)
    ...
    def forward(self, pre_seq, in_text, audio_feat_seq=None, vid_indices=None):
        decoder_hidden = None
        ...
        if self.input_context != 'none':
            # audio
            # audio_feat_seq = self.audio_encoder(in_audio, vid_indices)  # output (bs, n_frames, feat_size)
        ...
        if self.z_obj:
            if self.speaker_embedding:
                ...
                z_mu = z_logvar = None
                z_context = torch.randn(audio_feat_seq.shape[0], self.z_size, device=audio_feat_seq.device)
        else:
            z_mu = z_logvar = None
            z_context = None
...
class Hierarchical_Discriminator(nn.Module):
...
class Hierarchical_ConvDiscriminator(nn.Module):
