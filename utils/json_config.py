...
class JsonConfig(dict):
    ...
    def __init__(self, *argv, **kwargs):
        ...
        if isinstance(arg, str):
            ...
            meta_name = os.path.splitext(os.path.basename(arg))[0]
            with open(arg, "r") as load_f:
                arg = json.load(load_f)
            if "Meta" not in arg.keys():
                arg["Meta"] = {"name": meta_name}
            elif "name" not in arg["Meta"]:
                arg["Meta"]["name"] = meta_name
...
if __name__ == "__main__":
    config = JsonConfig("configs/cross-winlen30.json")
